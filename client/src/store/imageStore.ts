...
export interface ImageStoreState {
    ...
    setSelected: (file: ImageFile | null) => void;
    setChecked: (fileIds: number[]) => void;
    toggleChecked: (fileId: number) => void;
    setResizeSettings: (settings: Partial<ResizeSettings>) => void;
    setFileSize: (fileId: number, width: number, height: number) => void;
    reset: () => void;
}
...
export const useImageStore = create<ImageStoreState>((set) => ({
    ...
    setFileSize: (fileId, width, height) =>
        set((state) => ({
            files: state.files.map((f) => (f.id === fileId ? { ...f, width, height } : f)),
        })),
